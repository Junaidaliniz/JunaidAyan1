...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgresql://neondb_owner:npg_W6A9eOivmdXY@ep-curly-sea-ai3xk7dt-pooler.c-4.us-east-1.aws.neon.tech/neondb?sslmode=require")
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "add_panel_email":
        ...
        try:
            ...
            cur.execute(
                "INSERT INTO panels (name, login_url, base_url, sms_url, username, password, active) VALUES (%s, %s, %s, %s, %s, %s, TRUE)",
        ...
        try:
            ...
            if not cur.fetchone():
                cur.execute(
                    "INSERT INTO groups (group_id, panel, active, buttons) VALUES (%s, 'none', TRUE, '[]')",
                    (group_id,)
                )
                conn.commit()
                cache_invalidate("groups")
        ...
        try:
            ...
            if not cur.fetchone():
                cur.execute("INSERT INTO owners (user_id) VALUES (%s)", (owner_id,))
                conn.commit()
