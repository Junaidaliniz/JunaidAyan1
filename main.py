...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not owners:
        ...
        save_owners(owners)
        await update.message.reply_text(f"You are the first user! Auto-registered as owner.\nYour ID: {user_id}")
    if is_owner(user_id):
        keyboard = [
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "panel_list":
        ...
        groups = load_groups()
        if group_id in groups:
            groups[group_id]["panel"] = panel_val
            save_groups(groups)
        await show_group_detail(query, group_id)
...
async def delete_number_api(panel_name, number_id, client=None, csrf=None):
    panels = load_panels()
    panel_config = panels.get(panel_name)
    if not panel_config:
        return False
...
async def delete_all_numbers_from_range(query, panel_name, range_id, context):
    ...
    for i in range(0, total, batch_size):
        ...
        for entry in batch:
            ...
            if num_id:
                tasks.append(delete_number_api(panel_name, num_id, client, csrf))
        ...
        if done < total:
            await query.edit_message_text(f"\u23f3 Deleting... {done}/{total}")
    if real_range in grouped:
        del grouped[real_range]
        context.user_data[f"numbers_{panel_name}"] = grouped
    keyboard = [[InlineKeyboardButton("\u2b05\ufe0f Back to Ranges", callback_data=f"panel_numbers:{panel_name}")]]
...
async def get_panel_session(panel_name, panel_config):
    ...
    new_client = httpx.AsyncClient(timeout=4.0, follow_redirects=True, headers=headers, limits=limits, http2=False)
...
async def fetch_sms_from_panel(client, csrf_token, panel_config):
    ...
    try:
        ...
        summary_response = await client.post(sms_url_endpoint, data=first_payload, timeout=3.0)
        ...
        async def fetch_group(group_id):
            ...
            try:
                async with sem:
                    ...
                    numbers_response = await client.post(numbers_url, data=numbers_payload, timeout=3.0)
                ...
                async def fetch_number_sms(phone_number):
                    async with sem:
                        ...
                        try:
                            ...
                            sms_response = await client.post(sms_detail_url, data=sms_payload, timeout=3.0)
                            ...
                            flag = COUNTRY_FLAGS.get(country_name) or COUNTRY_FLAGS.get(country_name.title()) or COUNTRY_FLAGS.get(country_name.upper()) or COUNTRY_FLAGS.get(country_name.capitalize(), "\U0001f3f4\u200d\u2620\ufe0f")
...
async def check_sms_job(context: ContextTypes.DEFAULT_TYPE):
    global _job_running, _first_run_done, _processed_ids_cache
    if _job_running:
        return
    _job_running = True
    try:
        if not _processed_ids_cache:
            loaded = load_processed_ids()
            if loaded:
                _processed_ids_cache = loaded
                _first_run_done = True
                print(f"\U0001f504 Loaded {len(_processed_ids_cache)} processed IDs from file.")
                ...
                for msg in new_msgs:
                    ...
                    if sends:
                        ...
                        if any_success:
                            ...
                            _failed_msgs.append({"msg": msg, "groups": group_ids})
            except Exception as e:
        ...
        if _failed_msgs:
            ...
            for item in _failed_msgs:
                msg = item["msg"]
                if msg["id"] in _processed_ids_cache:
                    continue
                for gid in item["groups"]:
                    group_info = groups.get(gid, {})
                    group_buttons = group_info.get("buttons", [])
                    result = await send_telegram_message(context, gid, msg, buttons=group_buttons)
                    if result == "sent":
                        _processed_ids_cache.add(msg["id"])
                        total_new += 1
                        break
